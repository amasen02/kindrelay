...
export function App({ services }: { services: AppServices }) {
  const [items, setItems] = useState<
    ReadonlyArray<{ id: string; title: string; organization: string }>
  >([]);
  ...
  const create = async () => {
    ...
    setError(null);
    try {
      const handover = await services.repository.createHandover({
        title,
        organization,
      });
      setCurrent(handover);
  ...
  const open = async (id: string) => {
    ...
    setError(null);
    try {
      const handover = await services.repository.getHandover(id);
      if (!handover) throw new Error("Workspace was not found.");
      setCurrent(handover);
    } catch (reason) {
      setError(
  ...
  return (
    <main className="app-shell">
      ...
      <section aria-labelledby="create-workspace">
      ...
      <section aria-labelledby="workspaces">
        ...
        {loading ? (
          ...
          <ul>
            {items.map((item) => (
              <li key={item.id}>
                <button disabled={busy} onClick={() => void open(item.id)}>
                  {item.title}
                </button>{" "}
